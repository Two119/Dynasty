...
being_input = pygame_textinput.TextInputVisualizer(pygame_textinput.TextInputManager(validator=lambda input: ( len(input) <= 2 and input in [str(num_) for num_ in range(1, 100)]+[""])))
...
hunter_input = pygame_textinput.TextInputVisualizer(pygame_textinput.TextInputManager(validator=lambda input: ( len(input) <= 2 and input in [str(num_) for num_ in range(1, 100)]+[""])))
...
food_input = pygame_textinput.TextInputVisualizer(pygame_textinput.TextInputManager(validator=lambda input: ( len(input) <= 2 and input in [str(num_) for num_ in range(1, 51)]+[""])))
...
inputs = [being_input.surface.get_rect(topleft=[center_pos(title)[0]+20, center_pos(title)[1]/0.75]), hunter_input.surface.get_rect(topleft=[center_pos(title)[0]+20, (center_pos(title)[1]/0.75)+100]), food_input.surface.get_rect(topleft=[center_pos(title)[0]+20, (center_pos(title)[1]/0.75)+200])]
...
def back(args):
    global screen
    screen = 1
buttons = [Button([center_pos(title)[0]+20, center_pos(title)[1]/1.25], [scale_image(pygame.image.load("start_unpressed.png")), scale_image(pygame.image.load("start_pressed.png"))], [play, []])]
